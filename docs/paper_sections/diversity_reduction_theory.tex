...
This condition is increasingly well-satisfied as the steering magnitude $\|s\|$ grows (since $\|m\| \geq \|s\| - \|\mu\|$), which is precisely the regime where the diversity-reduction effect is strongest.
...
The output covariance is therefore:
%
\begin{equation}
  \Sigma_z
...
\end{equation}
%
Since $P_{\perp m}$ is symmetric ($P_{\perp m}^\top = P_{\perp m}$, because $\hat{m}\hat{m}^\top$ is an outer product of a vector with itself), this simplifies to:
%
\begin{equation}
